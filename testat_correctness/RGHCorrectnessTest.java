...
/**
 * RGH-Tree Index Correctness Test
 *
 * This test verifies that the RGH index returns the same results as brute-force search.
 * It compares the search results of both methods for multiple query points and radii.
 *
 * Run: java -cp "target/classes:libs/*" app.RGHCorrectnessTest
 */
public class RGHCorrectnessTest {
    ...
    public static void main(String[] args) throws IOException {
        System.out.println("===========================================");
        System.out.println("RGH-Tree Index Correctness Test");
        System.out.println("===========================================\n");

        RGHCorrectnessTest test = new RGHCorrectnessTest();
        test.run();
    }

    public RGHCorrectnessTest() {
        this.tableManager = TableManager.getTableManager("rgh_test");
    }
    ...
    public void run() throws IOException {
        ...
        try {
            // Build RGH index
            System.out.println("Building RGH index...");
            buildRGHIndex();
            System.out.println("RGH index built successfully.\n");
            ...
            for (IndexObject queryPoint : queryPoints) {
                for (double radius : TEST_RADII) {
                    ...
                    if (result.passed) {
                        passedTests++;
                        System.out.printf("  [PASS] Query: %s, Radius: %.2f -> RGH: %d results (dist:%d), BruteForce: %d results%n",
                                queryPoint.toString().substring(0, Math.min(30, queryPoint.toString().length())),
                                radius, result.rghResultCount, result.rghDistanceCount, result.bruteForceResultCount);
                    } else {
                        failedTests++;
                        System.out.printf("  [FAIL] Query: %s, Radius: %.2f -> RGH: %d results (dist:%d), BruteForce: %d results%n",
                                queryPoint.toString().substring(0, Math.min(30, queryPoint.toString().length())),
                                radius, result.rghResultCount, result.rghDistanceCount, result.bruteForceResultCount);
                        System.out.println("        Missing from RGH: " + result.missingResults);
                        System.out.println("        Extra in RGH: " + result.extraResults);
                    }
                }
                System.out.println();
            }
    ...
    /**
     * Build the RGH index using the system interface
     */
    private void buildRGHIndex() throws IOException {
        String tableName = IndexBuilder.buildRGHIndexOnVector(
                tableManager,
                TEST_DATA_FILE,
                DIMENSION,
                MAX_DATA_SIZE,
    ...
    private TestResult runTest(IndexObject queryPoint, double radius) {
        ...
        Set<Integer> rghResultIds = new HashSet<>();
        int rghDistanceCount = cursor.getDisCounter();
        result.rghDistanceCount = rghDistanceCount;
        while (cursor.hasNext()) {
            DoubleIndexObjectPair pair = cursor.next();
            rghResultIds.add(pair.getObject().getRowID());
            result.rghResultCount++;
        }
        ...
        for (IndexObject obj : allData) {
            ...
            if (distance <= radius) {
                ...
                if (!rghResultIds.contains(obj.getRowID())) {
        ...
        // Note: RGH should NOT return extra results, only missing results indicate bug
        result.passed = (result.rghResultCount == result.bruteForceResultCount && result.missingResults.isEmpty());
