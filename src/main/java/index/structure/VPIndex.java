...
public class VPIndex extends AbstractIndex
{
     ...
     */
    public VPIndex(String indexPrefix, List<? extends IndexObject> data, Metric metric, int maxLeafSize, int numPivot, int numPartitions, PivotSelectionMethod pivotSelectionMethod, PartitionMethod partitionMethod)
    {
    ...
    /**
     * 使用指定模式构建VP-tree。默认 specifyPivots 为 null
     *
     * @param indexPrefix                    输出的索引文件的名称
     * @param data                           用于构建索引的数据
     * @param metric                         构建索引使用的距离函数
     ...
     * @param partitionMethod                划分方法
     */
    public VPIndex(String indexPrefix, List<? extends IndexObject> data, Metric metric, int maxLeafSize, int numPivot, int numPartitions, HierarchicalPivotSelectionMode hierarchicalPivotSelectionMode, PivotSelectionMethod pivotSelectionMethod, PartitionMethod partitionMethod)
    {
     ...
     */
    public VPIndex(String indexPrefix, List<? extends IndexObject> data, Metric metric, int maxLeafSize, int numPivot, int numPartitions, HierarchicalPivotSelectionMode hierarchicalPivotSelectionMode, PivotSelectionMethod pivotSelectionMethod, PartitionMethod partitionMethod, IndexObject[] specifyPivots)
    {
    ...
    {
        if (q instanceof RangeQuery) return new VPRangeCursor((RangeQuery) q, oiom, metric, root);
    ...
    {
        StringBuilder toString = new StringBuilder("VPIndex{" + "hierarchicalPivotSelectionMode=" + hierarchicalPivotSelectionMode + ", root=" + root + ", totalSize=" + totalSize + ", maxLeafSize=" + maxLeafSize + ", metric=" + metric + ", indexPrefix='" + indexPrefix + '\'' + ", numPivot=" + numPivot + ", numPartitions=" + numPartitions);
    ...
    {
        return new VPRangeCursor(this.oiom, this.metric, this.root);
    }
}
