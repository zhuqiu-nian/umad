...
public class CPIndex extends AbstractIndex
{
    private static final long                 serialVersionUID = 8008240423051548569L;
    private final        PivotSelectionMethod pivotSelectionMethod;
    private final        PartitionMethod      partitionMethod;

    /**
     * 默认使用“LOCAL”模式建立CP-tree。
     *
     ...
     */
    public CPIndex(String indexPrefix, List<? extends IndexObject> data, Metric metric, int maxLeafSize, int numPivot, int numPartitions, PivotSelectionMethod pivotSelectionMethod, PartitionMethod partitionMethod)
    {
    ...
    /**
     * 使用指定模式构建CP-tree。默认 specifyPivots 为 null
     *
     * @param indexPrefix                    输出的索引文件的名称
     * @param data                           用于构建索引的数据
     * @param metric                         构建索引使用的距离函数
     ...
     * @param partitionMethod                划分方法
     */
    public CPIndex(String indexPrefix, List<? extends IndexObject> data, Metric metric, int maxLeafSize, int numPivot, int numPartitions, HierarchicalPivotSelectionMode hierarchicalPivotSelectionMode, PivotSelectionMethod pivotSelectionMethod, PartitionMethod partitionMethod)
    {
     ...
     */
    public CPIndex(String indexPrefix, List<? extends IndexObject> data, Metric metric, int maxLeafSize, int numPivot, int numPartitions, HierarchicalPivotSelectionMode hierarchicalPivotSelectionMode, PivotSelectionMethod pivotSelectionMethod, PartitionMethod partitionMethod, IndexObject[] specifyPivots)
    {
    ...
    {
        if (q instanceof RangeQuery) return new CPRangeCursor((RangeQuery) q, oiom, metric, root);
    ...
    {
        StringBuilder toString = new StringBuilder("GPIndex{" + "hierarchicalPivotSelectionMode=" + hierarchicalPivotSelectionMode + ", root=" + root + ", totalSize=" + totalSize + ", maxLeafSize=" + maxLeafSize + ", metric=" + metric + ", indexPrefix='" + indexPrefix + '\'' + ", numPivot=" + numPivot + ", numPartitions=" + numPartitions);
    ...
    {
        return new CPRangeCursor(this.oiom, this.metric, this.root);
    }
}
