...
import index.search.PowerDistanceRangeCursor;
...
public class PowerDistanceIndex extends AbstractIndex
{
    private static final long serialVersionUID = 9208240423051548570L;

    private final PivotSelectionMethod pivotSelectionMethod;
    private final PartitionMethod partitionMethod;

    public PowerDistanceIndex(String indexPrefix, List<? extends IndexObject> data, Metric metric,
    ...
    {
        this(indexPrefix, data, metric, maxLeafSize, 2, mode,
                pivotSelectionMethod, partitionMethod, specifyPivots);
    }

    public PowerDistanceIndex(String indexPrefix, List<? extends IndexObject> data, Metric metric,
                              int maxLeafSize, int numPartitions,
                              HierarchicalPivotSelectionMode mode,
                              PivotSelectionMethod pivotSelectionMethod,
                              PartitionMethod partitionMethod,
                              IndexObject[] specifyPivots)
    {
        super(indexPrefix, data, metric, maxLeafSize, 2, numPartitions, mode, specifyPivots);
    ...
    public void buildTree()
    {
        ...
        {
            throw new IllegalArgumentException("PowerDistanceIndex requires 2 pivots and at least 2 partitions");
        }
        super.buildTree();
    }
    ...
    public Cursor search(Query q)
    {
        ...
        {
            return new PowerDistanceRangeCursor((RangeQuery) q, oiom, metric, root);
        }
        else if (q instanceof KNNQuery)
        {
            throw new UnsupportedOperationException("PowerDistanceIndex does not support KNNQuery yet");
        }
        else
        {
    ...
    {
        return new PowerDistanceRangeCursor(this.oiom, this.metric, this.root);
    }
}
