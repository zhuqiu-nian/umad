...
public class PCAOnFFT implements PivotSelectionMethod, java.io.Serializable
{
    ...
    /* (non-Javadoc)
     * @see PivotSelectionMethod#selectPivots(metric.Metric, java.util.List, int)
     */
    public int[] selectPivots(Metric metric, List<? extends IndexObject> data, int numPivots)
    {
        ...
        if (dataSize < 10)
        {
            ...
            for (int i=0;i<dataSize; i++)
            {
                double []v = ( (db.DoubleVector)data.get(i)).getData();
        ...
        try
        {
            matrix = PCA.EMPCA(matrix, numPivots);
        } catch (Exception e)
        {
        ...
        int[] result = PCA.pivotSelectionByPCAResultAngle(matrix, numPivots);
    ...
    /* (non-Javadoc)
     * @see PivotSelectionMethod#selectPivots(metric.Metric, java.util.List, int, int, int)
     */
