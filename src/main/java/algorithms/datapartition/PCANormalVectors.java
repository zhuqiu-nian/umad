...
/*
 * https://blog.csdn.net/weixin_53972936/article/details/123300373
 *  算法步骤:
 * 1)将原始数据按列组成n行m列矩阵X
...
//      A.eig().getV().print(6, 2);
     ...
     * @return
     */
    public List<Vector<Double>> getVectors(double[][] data)
    {
        double[][] pcaData = null;
        if(data.length < 500)
            pcaData = data;
        ...
        {
            pcaData = new double[500][];
            Random random = new Random();
            for (int i = 0; i < pcaData.length; i++)
            {
                pcaData[i] = data[random.nextInt(data.length)];
            }
        }
        PCANormalVectors pcaNormalVectors = new PCANormalVectors();
        //均值0化后的数据
        double[][] averageArray = pcaNormalVectors.changeAverageToZero(pcaData);

        //协方差矩阵
        double[][] varMatrix = pcaNormalVectors.getVarianceMatrix(averageArray);

        //特征值矩阵
        double[][] eigenvalueMatrix = pcaNormalVectors.getEigenvalueMatrix(varMatrix);

        //特征向量矩阵
        double[][] eigenVectorMatrix = pcaNormalVectors.getEigenVectorMatrix(varMatrix);

        List<Vector<Double>> result = new LinkedList<>();
        if (eigenVectorMatrix == null)
        {
