...
public class Histogram
{
    ...
    public static class BinInfo
    {
        ...
        {
            if (width <= 0) throw new IllegalArgumentException("Bin width must be positive!");
        ...
        {
            return (!(value < center - width / 2)) && (!(value >= center + width / 2));
        }
    }
     ...
     * 计算给定的一维数据的直方图
     *
     * @param start “组”的开始边界，可以在开始点的左边
     * @param width “组”的宽度，必须是正值
     * @param dist  用来计算直方图的距离数组
     * @param first 第一个数据的下标（包含第一个数据）
     * @param last  最后一个数据的下标（不包含最后一个数据）
     * @return “组”构成的列表
     */
    public static ArrayList<BinInfo> completeOneDHistogram(double start, double width, double[] dist, int first, int last)
    {
        TreeMap<Integer, BinInfo> map = new TreeMap<Integer, BinInfo>();
        for (int i = first; i < last; i++)
        {
            double temp = (dist[i] - start) / width;
            int offset = (Math.abs(temp - Math.ceil(temp)) <= Delta) ? (int) Math.ceil(temp) : (int) Math.floor(temp);
            if (map.containsKey(offset))
            {
                map.put(offset, map.get(offset).addPoint(dist[i]));
            } else map.put(offset, (new BinInfo(start + (offset + 0.5) * width, width)).addPoint(dist[i]));
        }
     ...
     * @param start “组”的开始边界
     * @param width “组”的宽度
     * @param dist  用来计算直方图的距离数据
     * @return 一个2行的二维数组。第一行是每个"组"的上界，第二行是对应的“组”大小
     */
    ...
    /**
     * 给定二维 (x-y)值、范围和“组”数量，输出每个二维值出现的次数(非零)。
     *
     * @param xMin   x的最小值
     * @param xMax   x的最大值
     * @param xSize  x的“组”数，或者是说x的范围将会被划分为多少个部分
     * @param xValue x的值
     * @param yMin   y的最小值
     * @param yMax   y的最大值
     * @param ySize  y的“组”数，或者说y的范围将会被划分为多少个部分
     * @param yValue y的值
     * @return 一个三维向量，第一维是x值，第二维是y值，第三维是出现的次数
     */
    public static int[][] TwoDHist(double xMin, double xMax, int xSize, double[] xValue, double yMin, double yMax, int ySize, double[] yValue)
    {
        // check argument
        if (xValue.length != yValue.length) throw new IllegalArgumentException("x, y values of different length!");
     ...
     *
     * @param args the first should be the file name, then a series of numbers or chars, x0, x1, ..., xn,
    ...
    public static void pivotSpaceHistogram(String[] args) throws Exception
    {
        ...
        {
            if (args[i].equals(".")) toCompute[i - 1] = true;
        ...
        for (int j = 0; j < dataSize; j++)
        {
            ...
            for (int i = 0; i < pivotNumber; i++)
                if (toCompute[i]) keyBuffer.append((char) (int) Double.parseDouble(lineSegment[i]));

            key = new String(keyBuffer);
            if (map.containsKey(key)) map.put(key, map.get(key).add(BigInteger.ONE));
            else map.put(key, new BigInteger("1"));
        }
        ...
        for (int i = 0; i < pivotNumber; i++)
            if (isFixed[i]) System.out.print("pivot " + pivot[i] + " = " + fixedValue[i] + ", ");
        System.out.print(" ],     ");

        // find the min, max value
        int histDim = 0; // number of dimensions to compute histogram
        for (int i = 0; i < pivotNumber; i++)
            if (toCompute[i]) histDim++;
        ...
        while (p.hasNext())
        {
            ...
            for (int i = 0; i < histDim; i++)
            {
                if ((int) key.charAt(i) < min[i]) min[i] = key.charAt(i);
                if ((int) key.charAt(i) > max[i]) max[i] = key.charAt(i);
            }
        ...
        System.out.println("map size  = " + map.size() + ",   dataset size = " + sum + ",  pivot number = " + pivotNumber);
        // var /= map.size();

        System.out.print("#[min, max] values of each dim of histogram: ");
        for (int i = 0; i < histDim; i++)
             System.out.print("pivot " + pivot[mapping[i]] + ": [" + min[i] + ", " + max[i] + " ], ");
        System.out.println();
            ...
            if (((max[0] - min[0] + 1) > 0) && ((max[1] - min[1] + 1) > 0))
            {
                ...
                for (int i = 0; i < max[0] - min[0] + 1; i++)
                    for (j = 0; j < max[1] - min[1] + 1; j++)
                         histogram2D[i][j] = 0;
                System.out.println("#Histogram: " + " std. dev. = " + Math.sqrt(var / (histogram2D.length * histogram2D[0].length) - Math.pow(sum / (histogram2D.length * histogram2D[0].length), 2)) + ", std. dev. of non-zero buckets = " + Math.sqrt(var / map.size() - Math.pow(sum / map.size(), 2)) + ", maximum of histogram:" + maximum);
            }
        } else if (histDim == 1)
        {
            if ((max[0] - min[0] + 1) > 0)
            {
                ...
                for (int i = 0; i < max[0] - min[0] + 1; i++)
                     histogram1D[i] = 0;
                System.out.println("#Histogram: " + " std. dev. = " + Math.sqrt(var / histogram1D.length - Math.pow(sum / histogram1D.length, 2)) + ", std. dev. of non-zero buckets = " + Math.sqrt(var / map.size() - Math.pow(sum / map.size(), 2)) + ", maximum of histogram:" + maximum);
            }
        } else System.out.println("#Histogram: " + " std. dev. of non-zero buckets = " + Math.sqrt(var / map.size() - Math.pow(sum / map.size(), 2)) + ", maximum of histogram:" + maximum);
        ...
        while (p.hasNext())
        {
            ...
            if (histDim == 1)
            {
                histogram1D[(int) key.charAt(0) - min[0]] = ((BigInteger) entry.getValue()).intValue();
            } else if (histDim == 2)
            {
                histogram2D[(int) key.charAt(0) - min[0]][(int) key.charAt(1) - min[1]] = ((BigInteger) entry.getValue()).intValue();
            } else
            {
                System.out.print("(");
                for (int i = 0; i < histDim; i++)
     ...
     *
     * @param args the first should be the file name, the second and third should be sequential id of
     ...
     * screen.
     *
     * @param args the first should be the file name, then width of bin. then a series of numbers or
    ...
    public static String continuousPivotSpaceHistogram(String[] args, boolean print) throws Exception
    {
        // read file
        BufferedReader fileReader = new BufferedReader(new FileReader(args[0]));
        ...
        for (int i = 0; i < dataSize; i++)
        {
            ...
            for (int j = 0; j < histDim; j++)
            {
                ...
                if (distance[j][counter] < lowerBound[histMapping[j]]) lowerBound[histMapping[j]] = distance[j][counter];

                if (distance[j][counter] > upperBound[histMapping[j]]) upperBound[histMapping[j]] = distance[j][counter];
            }
        ...
        for (int i = 0; i < counter; i++)
        {
            ...
            for (int j = 0; j < histDim; j++)
            {
                ...
                if (histMap[j].containsKey(offsetKey)) histMap[j].put(offsetKey, histMap[j].get(offsetKey).add(BigInteger.ONE));
                else histMap[j].put(offsetKey, new BigInteger("1"));
        ...
        if (print)
        {
            ...
            for (int i = 0; i < fixedDim; i++)
                 System.out.println("pivot " + pivot[fixedMapping[i]] + " = (" + lowerBound[fixedMapping[i]] + " ~ " + upperBound[fixedMapping[i]] + ") ");

            System.out.println("[min, max, average, std.dev, entropy]: width values of each dimension to compute histogram: ");
        }

        for (int i = 0; i < histDim; i++)
        {
            if (print) System.out.println("pivot " + pivot[histMapping[i]] + ": [" + lowerBound[histMapping[i]] + ", " + distSum[i] + ", " + upperBound[histMapping[i]] + ", " + distVar[i] + ", " + histEntropy[i] + " ]:  " + width[histMapping[i]]);

            dimResult.append(pivot[histMapping[i]].split(":")[1].trim() + " [ " + (upperBound[histMapping[i]] - lowerBound[histMapping[i]]) + " : " + lowerBound[histMapping[i]] + " " + distSum[i] + " " + upperBound[histMapping[i]] + " " + distVar[i] + " " + histEntropy[i] + " ] ");
        }
        ...
        if (print)
        {
            System.out.println("Sum of variance, entropy, on all dimensions =" + distVar[0] + ", e: " + histEntropy[0]);

            System.out.println("map size  = " + map.size() + ",   dataset size = " + (int) sum + ",  pivot number = " + pivotNumber + ", max bin size = " + max);
        }
        ...
        if (print) System.out.println("Std. dev. among non-empty bins = " + Math.sqrt(var / map.size() - Math.pow(sum / map.size(), 2)) + ", among all bins = " + Math.sqrt(var / totalBin - Math.pow(sum / totalBin, 2)) + ", 2-raw moment = " + (var / counter) / counter + ", entropy = " + (Math.log(counter) - entropy / counter));

        result.append(" " + (Math.log(counter) - entropy / counter) + " " + histEntropy[0] + " " + (var / counter) / counter + " " + Math.sqrt(var / map.size() - Math.pow(sum / map.size(), 2)) + " " + Math.sqrt(var / totalBin - Math.pow(sum / totalBin, 2)) + " ");
        result.append(dimResult);

        if (!print) return result.toString();
        ...
        if (histDim == 2)
        {
            if ((upperBound[histMapping[0]] >= lowerBound[histMapping[0]]) && (upperBound[histMapping[1]] >= lowerBound[histMapping[1]]))
            {
        ...
        while (p.hasNext())
        {
            ...
            {
                histogram2D[(int) key.charAt(0)][(int) key.charAt(1)] = ((BigInteger) entry.getValue()).intValue();
            } else
            {
                /*
    ...
    static void aminoacid(String[] args) throws Exception
    {
        ...
        for (int i = 0; i < total; i++)
        {
            for (int j = i + 1; j < total; j++)
            {
                ...
                result.append(all.charAt(i)).append(all.charAt(j)).append(" : " + continuousPivotSpaceHistogram(parameter.toString().split(" ")) + "\n");
    ...
    public static double allPairs(String[] args, boolean print) throws Exception
    {
        ...
        for (int i = 0; i < dim - 1; i++)
        {
            for (int j = i + 1; j < dim; j++)
            {
                ...
                if (print) System.out.println(result.toString());
        ...
        for (int i = 0; i < total; i++)
        {
            for (int j = i + 1; j < total; j++)
            {
                for (int k = j + 1; k < total; k++)
                {
                    ...
                    result.append(all.charAt(i)).append(all.charAt(j)).append(all.charAt(k)).append(" : " + continuousPivotSpaceHistogram(parameter.toString().split(" ")) + "\n");
