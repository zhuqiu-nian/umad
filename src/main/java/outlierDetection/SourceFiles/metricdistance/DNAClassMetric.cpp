...
double CDNAClassMetric::EditDistanceMatrix[DNASYMBOLNUMBER][DNASYMBOLNUMBER] = 
{
...
CDNAClassMetric::CDNAClassMetric()
{
}
...
//Œﬁ“‚“Â
double CDNAClassMetric::getDistance(CMetricData* _obj1, CMetricData* _obj2, int length, int start, int end)
{
	CDNAClass* new_one = static_cast<CDNAClass*>(_obj1);
	CDNAClass* new_two = static_cast<CDNAClass*>(_obj2);
 ...
 */
double CDNAClassMetric::getDistance(CMetricData* one, CMetricData* two)
{
	CDNAClass* new_one = static_cast<CDNAClass*>(one);
	CDNAClass* new_two = static_cast<CDNAClass*>(two);
	return getDistance(new_one, new_two);
}
 ...
 * @return  Return a double type distance of two objects.
 */
double CDNAClassMetric::getDistance(CDNAClass* one, CDNAClass* two)
{
	int i;
	...
	{
		dist = dist + CDNAClassMetric::EditDistanceMatrix[one_byte[i]][two_byte[i]];
	}
	return dist;
}