...
#include "../../../HeaderFiles/outlierdetection/detectionalgorithm/DPPOD.h"
...
CDPPOD::CDPPOD(vector<shared_ptr<CMetricData>> &md, COutlierDefinition *_od, CMetricDistance *metrictype, int _n, int _k, int _b)
{
	metricData = &md;
...
CMetricDistance* CDPPOD::getMetric()
{
	return metric;
}


CKNN* CDPPOD::getOutlier(int outlierNum, int parameter)
{
	...
	CDensityPeakPivotSelection *psm1 = 0;
	...
	cout<<"borderPivots: "<<borderPivot[0]<<endl;
	...
	cout<<"indexTime:"<<indexStopTime-indexStartTime<<endl;
