#include "../../../HeaderFiles/outlierdetection/detectionalgorithm/PFOD.h"
...
#include "../../../HeaderFiles/util/Heap.h"
#include <functional>
#include "windows.h"
#include <psapi.h>
...
double getBTWeight(double *knn, int mNum, int kNum)
{
	double sum = 0;
	for(int i=mNum; i<mNum+kNum; i++)
	{
		sum += knn[i];
	}
	return sum;
...
CPFOD::CPFOD(vector<shared_ptr<CMetricData>> &md, COutlierDefinition *_od, CMetricDistance *metrictype, int _n, int _k, int _b, int _m)
{
	metricData = &md;
...
double CPFOD::getTempWeight(CKNN * knn, int kNum)
{
...
CMetricDistance* CPFOD::getMetric()
{
	return metric;
}

CKNN* CPFOD::getOutlier(int outlierNum, int parameter)
{
	...
	double *BlockKNN = new double[m+k];
	Heap<double,greater<double>> heap(BlockKNN, m+k);
	double *tempDegree = new double[size];
	int pruneNum = 0;
	int disComputeTimes = 0;
	vector<int> detectionObject;
	while(i<size)
	{
		B[j] = i;
		j++;
		...
		if((j == blockSize)||(i == size-1))
		{
			...
			//cout<<"blockPruneNum: "<<blockPruneNum<<endl;
			if((blockPruneNum == blockSize) && (i<size))
			{
				j = 0;
				i++;
				continue;
			...
			for(int d=0; d<size; d++)
			{
				...
				if(isFirstBlock)
				{
					heap.sortK(k);
					//heap.sort();
					tempDegree[d] = getBTWeight(BlockKNN, m, k);
					//cout<<tempDegree[d]<<endl;
				}
			}
			isFirstBlock = false;
			for(int b=0; b<j; b++)
	...
	double PFODAccuracy = getAccuracy(trueState, n);
	cout<<"PFODAccuracy:"<<PFODAccuracy<<endl;
	/*cout<<"trueState:"<<endl;
	for(int i=0; i<n; i++)
	{
		cout<<trueState[i];
